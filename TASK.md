...
|---|---|---|---|
| ✅ T-B7 (config) | `package.json` (build.mac/dmg/linux), `build/entitlements.mac.plist`, `tests/unit/build-config.test.js` | electron-builder mac (dmg+zip, hardenedRuntime, productivity category, entitlements) + linux (AppImage+deb, Office) targets; entitlements grant user-selected-folder + JIT access. Config validated by a static test. ⚠️ Producing/smoke-testing the mac/linux ARTIFACTS is platform-blocked on win32 (needs macOS/Linux or the CI/Docker pass); **code-signing/notarization is skipped without certs** (electron-builder no-ops signing absent a CSC identity — Q5). | Q5 signing/notarization |
