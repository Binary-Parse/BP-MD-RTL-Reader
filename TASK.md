...
|---|---|---|---|
| ✅ T-B7 (config) | `package.json` (build.mac/dmg/linux), `build/entitlements.mac.plist`, `tests/unit/build-config.test.js` | electron-builder mac (dmg+zip, hardenedRuntime, productivity category, entitlements) + linux (AppImage+deb, Office) targets; entitlements grant user-selected-folder + JIT access. Config validated by a static test. ✅ **Artifact build now wired in CI** — `.github/workflows/release.yml` is a tag-triggered (`v*`) 3-OS matrix (windows/macos/ubuntu-latest) that runs `electron-builder --publish always` on each platform's native runner (unblocks the win32-only dev box) and uploads to the GitHub Release. **Code-signing/notarization is opt-in:** the workflow wires `CSC_LINK`/`CSC_KEY_PASSWORD` (Win) + `APPLE_ID`/`APPLE_APP_SPECIFIC_PASSWORD`/`APPLE_TEAM_ID` (mac) from repo secrets — electron-builder signs when they're set, ships unsigned otherwise (Q5 still needs real certs uploaded as secrets to actually sign). | Q5 signing/notarization (certs) |
...
| ✅ Q6 | `src/main/version.js` (compareVersions), `main.js` (`update:check`), `preload.js` (`checkForUpdate`), `src/renderer/app.js` (Help ▸ Check for Updates…), `tests/{unit/version,unit/main-update-check,q6-update-check}` | opt-in update-CHECK only — user-initiated (Help menu); GETs the public releases manifest + version-compares; NO auto-check on launch, NO auto-download, NO identifiers/telemetry sent. Typed errors (network/http/parse/no-version/unsupported); injected fetch for tests. The only outbound request the app ever makes, and only on user action. | EC-D3 |
...
**Test status (after the autonomous polish pass):** 866 unit tests green · full Playwright e2e green on win32 · eslint 0 errors. Shipped this pass: **F1/M3 collapsible folder tree**, **M6 session restore**, **R7 catalog complete** (welcome + palette + find/search), **AI2/B4 viewer routed through the hardened sanitizer** (inline `style=` now actually forbidden — `FORBID_ATTR`), **F10/B10 cosmetics**, a **real-engine CM6 perf benchmark** (≈18 ms/3 ms on 10k lines), the **F13 LinkMark autolink/image guards**, and **A1 — CM6 behind a persisted `cmEditor` setting** (default off; live View-menu + palette toggle; restores on launch; case-D asserts the setting governs the mode). The agent leg landed per-line RTL + logical caret in CM6 (`4071b34`) and a single CM6 live-preview mode behind `?cm=1` (`373c8ee`).

**SC10 — mutation score CERTIFIED (full-repo `stryker run`):** overall **90.74% ≥ 90 break threshold ✓**. Per-module: preload/state/theme/main-logic/file-predicates **100%**, search 97.01, session 96.23, edit-commands 95.79, markdown 94.21, i18n 93.22; the only laggard is **main.js 83.99%** (85 surviving mutants — the Electron main process, a pre-existing gap). The certification fix added Stryker-traced coverage for `isVaultFile`/`isDroppableFile` in `main-logic.test.js` (the old `file-predicates.test.js` used `createRequire`, which Stryker's per-test instrumentation can't trace → 16 NoCoverage mutants), taking main-logic 83.51 → 100 and the overall 89.54 → 90.74.

**Remaining (deliberate / external):** make CM6 the DEFAULT editor (a step beyond A1's opt-in setting — maintainer-gated, churns the textarea-default suite); Q5 code-signing/notarization (real certs as CI secrets — off-box); and hardening `main.js` mutation coverage (pre-existing, the largest single mutation gap).
...
Working order: F16 ✅ → F4+F5 ✅ → T4+T5 ✅ → B6+F6 ✅ → B3-finish/T1/T3/B4 ✅ → R10 ✅ → F11+T6 ✅ → B9 ✅ → R7 ✅(core) → F12 ✅ → F13 ◐(CM6 foundation; live-preview next) → B7 ✅(config; artifacts platform-blocked) + Q6 ✅. **ALL 12 tasks addressed.** Remaining follow-ups: F13 live-preview UX, R7 string-catalog expansion, mac/linux artifact build + signing (Q5), and the Linux visual-baseline Docker pass. Linux visual baselines deferred to one final regeneration pass (Playwright v1.60.0-jammy Docker).
