...
// v10 redesign fix: selectAll is routed by VIEW mode (reading/edit), not editor mode.
// CM6 mounts unconditionally at launch, so a CM6 adapter exists even in Reading mode,
// where it is hidden behind #noteContent — the three cases below (Reading mode, the
// welcome screen, and focus left on chrome) were previously uncovered by any test.
test.describe('Ctrl+A / selectAll is scoped by view mode (v10 redesign)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(FILE_URL);
    await page.waitForSelector('.app', { state: 'visible' });
  });
  ...
  test('Reading mode: Ctrl+A selects the visible render, not the hidden CM6 editor', async ({ page }) => {
    ...
    const info = await page.evaluate(() => {
      ...
      return {
        text: sel.toString(),
        withinNoteContent: noteContent ? noteContent.contains(range.commonAncestorContainer) : false,
        withinSidebar: sidebar ? sidebar.contains(range.commonAncestorContainer) : false,
      };
    });
    expect(info.text).toContain('bold');
    expect(info.withinNoteContent).toBe(true);
    expect(info.withinSidebar).toBe(false);
  });
  ...
  test('focus left on a chrome control: Ctrl+A selects the document, not the sidebar', async ({ page }) => {
    await injectAndRender(page, '# Test\n\nSome **bold** text here.');
    ...
    const info = await page.evaluate(() => {
      ...
      const range = sel.getRangeAt(0);
      const inside = (selector) => { const el = document.querySelector(selector); return el ? el.contains(range.commonAncestorContainer) : false; };
      return {
        withinSidebar: inside('.sidebar'),
        withinTitlebar: inside('.titlebar'),
        withinStatusbar: inside('.statusbar'),
      };
    });
    expect(info.withinSidebar).toBe(false);
    expect(info.withinTitlebar).toBe(false);
    expect(info.withinStatusbar).toBe(false);
  });
});
