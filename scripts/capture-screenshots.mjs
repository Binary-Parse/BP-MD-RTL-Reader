...
await page.evaluate(({ en, ar }) => {
  ...
  s.files = [
    { name: 'the-slow-web.md', path: 'essays/the-slow-web.md', handle: null, content: en, dirty: false },
    { name: 'مقالة-القراءة.md', path: 'essays/مقالة-القراءة.md', handle: null, content: ar, dirty: false },
    { name: 'reading-list.md', path: 'reading-list.md', handle: null, content: '# Reading list\n', dirty: false },
  ];
  // Populate the sidebar. Without this the tree keeps its "No folder opened" empty
  // state, so every screenshot advertised an empty file list beside a full document.
  window.renderTree(s.files);
  window.renderFile(0);
  // The packaged app opens in Reading mode (src/main/settings.js). The renderer's own
  // in-memory default is Edit, so a file:// capture would otherwise show the raw
  // Markdown source in every "theme" shot and misrepresent the default appearance.
  window.setViewMode('reading');
}, { en: ENGLISH, ar: ARABIC });
// Hide transient toast notifications so they never pollute a screenshot, and suppress
// focus rings: #noteContent carries tabindex="0", so switching view modes focuses it and
// draws a keyboard focus outline around the whole reading pane. The ring is correct
// behaviour and must stay in the app — it just is not what a documentation still should
// show, since a real user arriving by mouse never sees it.
await page.addStyleTag({
  content: '#toast{opacity:0!important;visibility:hidden!important}'
...
// #themeBtn, whose designed [data-tip] tooltip then renders into each later capture.
...
// Editor view on the English note (auto-detects LTR). setViewMode('edit') is required:
// the app opens in Reading mode by default (main/settings.js), so rendering the note
// alone would capture the reading surface again and label it "editor".
