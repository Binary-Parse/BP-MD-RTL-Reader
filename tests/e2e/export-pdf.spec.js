...
test.describe('[T-F6] Export PDF action', () => {
  ...
  test('a Windows backslash path is shown as just the filename in the success toast', async ({ page }) => {
    await page.evaluate(() => {
      window.electronAPI = { exportPDF: () => Promise.resolve({ ok: true, path: 'C:\\Users\\me\\Documents\\report.pdf' }) };
      window.loadDemo();
    });
    await page.waitForTimeout(150);
    await page.evaluate(() => { window.renderFile(0); return window.exportPDF(); });
    await expect(page.locator('#toast')).toContainText('report.pdf');
