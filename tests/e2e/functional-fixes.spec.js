 ...
 *   R10  note-relative images are rewritten to bpmd://vault/<vaultId>/<rel>
 *
...
test.describe('[R10] note-relative images rewrite to bpmd://vault/<vaultId>/<rel>', () => {
  test('a root-level note rewrites ![](pic.png) to bpmd://vault/<vaultId>/pic.png', async ({ page }) => {
    await goto(page);
    ...
    expect(src).toBe('bpmd://vault/cap-vault/pic.png');
  });
  ...
  test('a note in a subfolder resolves the image against the note directory', async ({ page }) => {
    ...
    expect(src).toBe('bpmd://vault/cap-vault/sub/pic%20a.png');
  });

  // B1 (multi-folder workspaces): two notes from different folders must embed their
  // OWN vault's id — a shared bare "bpmd://vault/<rel>" URL (no id) would let the
  // protocol handler resolve either note's images against whichever folder was read
  // last, serving folder B's image under folder A's note.
  test('two notes from different vaults embed their own vaultId, not a shared root', async ({ page }) => {
    await goto(page);
    const srcs = await page.evaluate(() => {
      window._appState.files = [
        { name: 'a.md', path: 'a.md', content: '![](pic.png)', dirty: false, vaultId: 'cap-vault-a' },
        { name: 'b.md', path: 'b.md', content: '![](pic.png)', dirty: false, vaultId: 'cap-vault-b' },
      ];
      window.renderFile(0);
      const a = document.querySelector('#noteContent img')?.getAttribute('src');
      window.renderFile(1);
      const b = document.querySelector('#noteContent img')?.getAttribute('src');
      return { a, b };
    });
    expect(srcs.a).toBe('bpmd://vault/cap-vault-a/pic.png');
    expect(srcs.b).toBe('bpmd://vault/cap-vault-b/pic.png');
  });
