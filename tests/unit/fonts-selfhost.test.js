...
describe('self-hosted fonts (T-B3 / T1 / T3)', () => {
  test('no Google Fonts (or any CDN) font reference remains', () => {
    expect(html + appCss).not.toMatch(/fonts\.googleapis\.com/);
    expect(html + appCss).not.toMatch(/fonts\.gstatic\.com/);
    expect(html).not.toMatch(/<link[^>]+rel="preconnect"/);
  });

  test('declares @font-face for all four families', () => {
    for (const fam of ['Literata', 'Inter', 'JetBrains Mono', 'IBM Plex Sans Arabic']) {
      const re = new RegExp(`@font-face\\s*\\{[^}]*font-family:\\s*'${fam}'`, 's');
      expect(appCss, `missing @font-face for ${fam}`).toMatch(re);
    }
  });
  ...
  test('T1: variable families declare a weight RANGE and Arabic declares 400/500/600/700', () => {
    // Literata variable must span at least 200→900 (its full weight axis).
    expect(appCss).toMatch(/font-family:\s*'Literata'[^}]*font-weight:\s*200\s+900/s);
    ...
    for (const w of [400, 500, 600, 700]) {
      ...
      expect(appCss, `missing IBM Plex Arabic weight ${w}`).toMatch(re);
    }
  });

  test('T2: font-synthesis:none is set (no faked bold/italic)', () => {
    expect(appCss).toMatch(/font-synthesis:\s*none/);
  });

  test('T6: optical sizing is explicitly enabled (Literata opsz engaged; guards against a future :none)', () => {
    ...
    expect(appCss).toMatch(/font-optical-sizing:\s*auto/);
  });
  ...
  test('families used with font-style:italic ship a REAL italic face (font-synthesis:none → no faux-oblique)', () => {
    ...
    // are --serif (Literata). All three are used italic somewhere, so each needs an italic face.
    for (const fam of ['Literata', 'Inter', 'JetBrains Mono']) {
      const re = new RegExp(`@font-face\\s*\\{[^}]*font-family:\\s*'${fam}'[^}]*font-style:\\s*italic`, 's');
      expect(appCss, `${fam} is used italic but ships no italic @font-face`).toMatch(re);
    }
  });
});
