...
test.describe('[AC1] Open Folder IPC bridge', () => {
  ...
  test('[AC1-happy] mock electronAPI.openFolder populates State.files and renders sidebar', async ({ page }) => {
    ...
    // v10 redesign (2026-08-25): #vaultName (the sidebar header) is removed; the
    // status-bar stat is the surviving folder indicator (no "vault" text).
    const folderStat = await page.evaluate(() => document.getElementById('sbVault').textContent);
    expect(folderStat).toBe('folder: Notes');
    expect(folderStat.toLowerCase()).not.toContain('vault');
  });
...
test.describe('[AC2] No "vault" in user-facing strings', () => {
  ...
  // Black-box: check all data-tip= and aria-label= attributes (v10: tooltips moved off title=)
  test('[AC2-attr] no data-tip or aria-label attribute contains "vault" (case-insensitive)', async ({ page }) => {
    await goto(page);

    const vaultInAttrs = await page.evaluate(() => {
      const vaultRe = /vault/i;
      const elems = document.querySelectorAll('[data-tip],[aria-label]');
      const found = [];
      elems.forEach(el => {
        const tip = el.getAttribute('data-tip') || '';
        const aria = el.getAttribute('aria-label') || '';
        if (vaultRe.test(tip))  found.push({ attr: 'data-tip', val: tip, tag: el.tagName });
  ...
  // v10 redesign (2026-08-25): #sbOpenVaultBtn (the vault header) is removed; the
  // empty-state's Open Folder button is the surviving sidebar affordance.
  test('[AC2-sidebar-button] sidebar Open Folder button has no "vault" text', async ({ page }) => {
    await goto(page);
    const btnText = await page.evaluate(() => document.getElementById('emptyOpenVault').textContent);
    expect((btnText || '').toLowerCase()).not.toContain('vault');
    expect((btnText || '').toLowerCase()).toContain('folder');
  });
...
test.describe('[AC5] Tab overflow ellipsis and close-X visibility', () => {
  ...
  // Black-box: tab element has data-tip attribute equal to full filename
  test('[AC5-happy] tab element data-tip attribute equals full filename', async ({ page }) => {
    ...
    const tipAttr = await page.evaluate(() => {
      ...
      return tab ? tab.getAttribute('data-tip') : null;
    });
    expect(tipAttr).toBe(LONG_FILENAME);
  });
  ...
  // White-box: tab.dataset.tip set on the tab DIV, not the .tab-name span
  test('[AC5-wb-title-on-tab-div] data-tip attribute is on .tab element, not .tab-name span', async ({ page }) => {
    ...
    const spanTip = await page.evaluate(() => {
      const span = document.querySelector('.tab .tab-name');
      return span ? span.getAttribute('data-tip') : null;
    });

    expect(tabTip).toBe('my-note.md');
    // The tip should be on the parent tab, not the span (it might or might not be on span too)
    // Primary assertion: tab div has the tip
    expect(tabTip).not.toBeNull();
  });
