...
describe('src/preload/index.js', () => {
  ...
  test('exposes electronAPI methods (incl. logError #15, exportPDF T-F6)', () => {
    ...
    expect(typeof api.onContextMenu).toBe('function');
    expect(typeof api.contextAction).toBe('function');
    expect(typeof api.contextMenuClosed).toBe('function');
    expect(typeof api.onAppCommand).toBe('function');
  });

  // ─ v10 redesign (D1): renderer-drawn context menu bridge ──────────────────
  test('onContextMenu subscribes to context-menu:show and unwraps the payload', () => {
    mockElectron.ipcRenderer.on.mockClear();
    const seen = [];
    getApi().onContextMenu((payload) => seen.push(payload));
    const call = mockElectron.ipcRenderer.on.mock.calls.find((c) => c[0] === 'context-menu:show');
    expect(call, 'should subscribe to context-menu:show').toBeTruthy();
    call[1]({}, { nonce: 'n1', descriptors: [], x: 1, y: 2 });
    expect(seen).toEqual([{ nonce: 'n1', descriptors: [], x: 1, y: 2 }]);
  });

  test('contextAction sends context-menu:action with the { nonce, index } payload verbatim', () => {
    mockElectron.ipcRenderer.send.mockClear();
    getApi().contextAction({ nonce: 'n1', index: 2 });
    expect(mockElectron.ipcRenderer.send).toHaveBeenCalledWith('context-menu:action', { nonce: 'n1', index: 2 });
    expect(mockElectron.ipcRenderer.send).toHaveBeenCalledTimes(1);
  });

  test('contextMenuClosed sends context-menu:closed with the nonce', () => {
    mockElectron.ipcRenderer.send.mockClear();
    getApi().contextMenuClosed('n1');
    expect(mockElectron.ipcRenderer.send).toHaveBeenCalledWith('context-menu:closed', { nonce: 'n1' });
  });

  test('onAppCommand subscribes to app:command and unwraps the command', () => {
    mockElectron.ipcRenderer.on.mockClear();
    const seen = [];
    getApi().onAppCommand((cmd) => seen.push(cmd));
    const call = mockElectron.ipcRenderer.on.mock.calls.find((c) => c[0] === 'app:command');
    expect(call, 'should subscribe to app:command').toBeTruthy();
    call[1]({}, 'newNote');
    expect(seen).toEqual(['newNote']);
  });
...
describe('src/main/index.js — handler behaviour (audit #1)', () => {
  ...
  test('edit:command dispatches the 5 clipboard/undo commands to webContents', () => {
    const wc = mockElectron._mockWin.webContents;
    for (const m of ['copy', 'cut', 'paste', 'undo', 'redo']) {
      wc[m].mockClear();
    }
  ...
  // selectAll is intentionally NOT in the edit:command gate: webContents.selectAll()
  // selects the entire renderer DOM (titlebar/sidebar/statusbar), not just the document
  // (edit-commands.js's module header explains why this must never be forwarded). Nothing
  // sends 'selectAll' over this channel today, but the gate must stay closed.
  test('edit:command does not forward selectAll to webContents', () => {
    const wc = mockElectron._mockWin.webContents;
...
// v10 redesign (2026-08-25, D1): src/main/index.js no longer pops up a native Electron
// Menu here — it stashes the descriptor array under a single-use nonce and sends
// 'context-menu:show' so the renderer can draw a themed HTML menu (see
// tests/unit/main-window-contextmenu.test.js for the full IPC-round-trip contract). This
// file's own copy of the same contract is retargeted rather than deleted, since it's a
// genuine 4th consumer of the removed Menu.buildFromTemplate/.popup() mechanism that the
// v10 audit's file search (M19) missed.
describe('src/main/index.js — right-click context menu', () => {
  ...
  function lastShowPayload() {
    const call = mockElectron._mockWin.webContents.send.mock.calls.filter(c => c[0] === 'context-menu:show').at(-1);
    return call && call[1];
  }
  ...
  test('editable field → full edit menu with roles, enabled per editFlags', () => {
    ...
    expect(descriptors.filter(i => i.kind === 'role').map(i => i.role)).toEqual(['undo', 'redo', 'cut', 'copy', 'paste', 'selectAll']);
  ...
  test('non-editable WITH selection → Copy + Select All only', () => {
    ...
    const { descriptors } = lastShowPayload();
    expect(descriptors.filter(i => i.kind === 'role').map(i => i.role)).toEqual(['copy', 'selectAll']);
  });
