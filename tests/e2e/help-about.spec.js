...
test.describe('[HELP] Help menu + About attribution', () => {
  ...
  test('About dialog shows publisher "Binary Parse" and "MIT License"', async ({ page }) => {
    ...
    // Release polish: the About dialog states the real release version, not a prototype
    // label. Derived from package.json rather than pinned to a literal, so a version bump
    // that forgets the dialog fails here instead of shipping a dialog that lies.
