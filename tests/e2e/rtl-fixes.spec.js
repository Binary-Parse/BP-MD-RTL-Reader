...
test.describe('RTL and theme bug fixes', () => {
  ...
  test('[Statusbar-ink] statusbar has dark background in ink theme', async ({ page }) => {
    ...
    // In ink theme, --paper-deep is #1E222A = rgb(30, 34, 42) (v10 redesign, 2026-08-25).
    // The light ink value (#DDD7C9 = rgb(221, 215, 201)) must NOT be the background.
    ...
    // Dark background: sum < 200 (e.g. rgb(30,34,42) = 106)
    // Light background: sum > 500 (e.g. rgb(221,215,201) = 637)
    expect(brightness).toBeLessThan(200);
  });

  test('[Statusbar-paper] statusbar is integrated with the paper theme, not a dark foreign strip', async ({ page }) => {
    // v10 redesign (2026-08-25): the status bar used to invert to --ink regardless of
    // theme (a dark strip even under the light paper theme). The redesign integrates it
    // with the theme instead -- --statusbar-bg now resolves to --paper-deep everywhere,
    // so under paper it stays light.
    await page.goto(INDEX_URL);
    await page.waitForLoadState('networkidle');
    ...
    const [r, g, b] = match.map(Number);
    const brightness = r + g + b;
    // Paper --paper-deep is #EEE8D8 = rgb(238, 232, 216), sum = 686 (light)
