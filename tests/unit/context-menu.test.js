...
describe('buildContextMenuTemplate', () => {
  ...
  test('non-http link → no open and no copy (EC-B5)', () => {
    const t = buildContextMenuTemplate({ linkURL: 'javascript:alert(1)', isEditable: false }, deps);
    expect(ids(t)).not.toContain('copy-link');
    expect(ids(t)).not.toContain('open-link');
  });

  test('image → copy bitmap; address/save only for http(s) src', () => {
    const t = buildContextMenuTemplate({ mediaType: 'image', srcURL: 'bpmd://vault/p.png', isEditable: false }, deps);
    expect(ids(t)).toContain('copy-image');
    expect(ids(t)).not.toContain('copy-image-address');
    expect(ids(t)).not.toContain('save-image');
  });
...
describe('buildContextMenuTemplate — link context (exact)', () => {
  ...
  test('non-openable link (e.g. javascript:) → neither open-link nor copy-link', () => {
    const m = buildContextMenuTemplate({ linkURL: 'javascript:evil' }, D);
    expect(m.find((i) => i.id === 'open-link')).toBeUndefined();
    expect(m.find((i) => i.id === 'copy-link')).toBeUndefined();
  });
  test('missing isExternallyOpenable dep → treated as not openable (no open-link or copy-link)', () => {
...
describe('buildContextMenuTemplate — image context (exact)', () => {
  test('image with http srcURL → copy-image, copy-image-address, save-image (exact labels)', () => {
    const m = buildContextMenuTemplate({ mediaType: 'image', srcURL: 'https://img.example/p.png' }, D);
    expect(m.find((i) => i.id === 'copy-image')).toEqual({ kind: 'action', id: 'copy-image', label: 'Copy Image' });
    expect(m.find((i) => i.id === 'copy-image-address')).toEqual({ kind: 'action', id: 'copy-image-address', label: 'Copy Image Address', url: 'https://img.example/p.png' });
    expect(m.find((i) => i.id === 'save-image')).toEqual({ kind: 'action', id: 'save-image', label: 'Save Image', url: 'https://img.example/p.png' });
  });
  test('image WITHOUT srcURL → copy-image only', () => {
    const m = buildContextMenuTemplate({ mediaType: 'image' }, D);
    expect(m.find((i) => i.id === 'copy-image-address')).toBeUndefined();
    expect(m.find((i) => i.id === 'save-image')).toBeUndefined();
  });
});
